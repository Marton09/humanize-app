...
export default function LandingPage() {
  ...
  const t = {
    ...
    navBg:     scrolled ? (d ? "bg-[#0a0a0a]/88 backdrop-blur-2xl border-b border-white/[0.06]" : "bg-white/88 backdrop-blur-2xl border-b border-gray-200") : "bg-transparent",
    navLink:   d ? "text-white/50 hover:text-white"          : "text-gray-500 hover:text-gray-900",
    toggleBtn: d ? "bg-white/[0.07] hover:bg-white/[0.12] text-white/55 hover:text-white/85" : "bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-800",
    ...
    outlineCta:d ? "border-2 border-white/15 text-white/65 hover:border-orange-500/50 hover:text-white" : "border-2 border-gray-200 text-gray-600 hover:border-orange-300 hover:text-gray-900",
    footer:    d ? "border-white/[0.07]"                     : "border-gray-200",
    socialBtn: d ? "border-white/[0.07] text-white/35 hover:border-orange-500/30 hover:text-orange-400 hover:bg-orange-500/[0.06]" : "border-gray-200 text-gray-400 hover:border-orange-300 hover:text-orange-500 hover:bg-orange-50",
  };
      ...
      <nav className={`fixed top-8 left-0 right-0 z-50 transition-all duration-300 ${t.navBg}`}>
        <div className="max-w-[1160px] mx-auto px-6 py-[14px] flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className={`text-[15px] font-bold tracking-tight ${d ? "text-white/95" : "text-gray-900"}`}>HumanizeIt</span>
          </Link>
          <div className="hidden md:flex items-center gap-8">
            {[["Features", "#features"], ["Pricing", "#pricing"], ["How It Works", "#how-it-works"], ["Login", "/login"]].map(([label, href]) => (
              <Link key={label} href={href} className={`text-sm font-medium transition-colors duration-150 ${t.navLink}`}>{label}</Link>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setDark(!dark)} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${t.toggleBtn}`} aria-label="Toggle dark/light mode">
              {dark ? <SunIcon /> : <MoonIcon />}
            </button>
            <Link href="/signup" className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm ${orangeBtn}`}>Get Started</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="relative z-10 pt-8">
        ...
        <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 pt-28 pb-24">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Link href="/app" className={`flex items-center gap-2.5 px-9 py-[15px] rounded-2xl text-base ${orangeBtn}`}>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1L9.8 6.2H15.2L10.7 9.4L12.5 14.6L8 11.4L3.5 14.6L5.3 9.4L0.8 6.2H6.2L8 1Z" fill="currentColor"/></svg>
              Start for Free
            </Link>
        ...
        <section className="px-6 pb-28">
          <div className="max-w-[1000px] mx-auto">
            ...
            <div className={`grid grid-cols-2 rounded-3xl border overflow-hidden ${t.card}`} style={d ? { boxShadow: "0 0 0 1px rgba(249,115,22,0.12), 0 0 70px rgba(249,115,22,0.09)" } : { boxShadow: "0 4px 50px rgba(249,115,22,0.07), 0 0 0 1px rgba(249,115,22,0.08)" }}>
              <div className={`p-9 border-r ${d ? "border-white/[0.07]" : "border-gray-100"}`}>
                ...
                <p className={`text-[14.5px] leading-[1.9] ${t.muted}`}>The implementation of artificial intelligence technologies in contemporary business environments has demonstrated substantial potential for the optimization of operational efficiency metrics. It is furthermore noteworthy that the strategic utilization of machine learning algorithms has yielded considerable improvements in predictive analytical capabilities.</p>
              </div>
              <div className="p-9">
                <div className="flex items-center gap-2 mb-5">
                ...
                <p className={`text-[14.5px] leading-[1.9] ${d ? "text-white/80" : "text-gray-700"}`}>AI is reshaping how businesses operate — and the results are hard to ignore. Teams leaning into machine learning aren&#39;t just saving time, they&#39;re catching problems before they even surface. That&#39;s a genuine edge, and more companies are waking up to it every single day.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="px-6 pb-28">
          <div className="max-w-[1000px] mx-auto">
            ...
            <div className="grid grid-cols-3 gap-5 relative">
              ...
              {STEPS.map((s) => (
                <div key={s.num} className={`flex flex-col items-center text-center px-8 py-9 rounded-2xl border transition-all duration-200 ${t.step} ${t.stepHover}`}>
                  <div className="w-[52px] h-[52px] rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center mb-5 shadow-lg shadow-orange-900/30 text-white text-sm font-black">{s.num}</div>
        ...
        <section id="features" className="px-6 pb-28">
          <div className="max-w-[1000px] mx-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              {FEATURES.map((f) => (
                <div key={f.title} className={`flex items-start gap-5 p-7 rounded-2xl border cursor-default transition-all duration-200 ${t.card} ${t.cardHover}`}>
                  <div className="shrink-0 w-11 h-11 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400"><f.Icon /></div>
                  <div>
        ...
        <section id="pricing" className="px-6 pb-28">
          <div className="max-w-[1240px] mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className={`text-[2.2rem] font-black tracking-tight mb-3 ${d ? "text-white" : "text-gray-900"}`}>Simple, Transparent Pricing</h2>
              <p className={`text-base ${t.sub}`}>Start free. Upgrade anytime, cancel anytime.</p>
            </div>
            <div className="flex items-center justify-center gap-3 mb-12">
              <span className={`text-sm font-semibold transition-colors duration-200 ${!annual ? (d ? "text-white/90" : "text-gray-900") : t.muted}`}>Monthly</span>
              <button onClick={() => setAnnual(!annual)} aria-label="Toggle billing period" className={`relative w-12 h-6 rounded-full transition-colors duration-300 focus:outline-none ${annual ? "bg-orange-500" : d ? "bg-white/[0.12]" : "bg-gray-200"}`}>
              ...
              <span className={`text-sm font-semibold transition-colors duration-200 ${annual ? (d ? "text-white/90" : "text-gray-900") : t.muted}`}>Annual</span>
              <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all duration-300 ${annual ? "bg-emerald-500/12 text-emerald-400 border-emerald-500/25 opacity-100 scale-100" : "opacity-0 scale-90 pointer-events-none bg-transparent border-transparent text-transparent"}`}>2 MONTHS FREE</span>
            </div>
            <div className="grid grid-cols-4 gap-4 items-start">
              <div className={`rounded-2xl border p-7 flex flex-col transition-all duration-200 ${t.card} ${t.cardHover}`}>
                ...
                <Link href="/app" className={`w-full flex items-center justify-center py-3 rounded-xl text-sm font-bold border-2 transition-all duration-200 ${d ? "border-white/[0.12] text-white/50 hover:border-orange-500/35 hover:text-white/80" : "border-gray-200 text-gray-500 hover:border-orange-300 hover:text-gray-900"}`}>Get Started Free</Link>
              </div>
              <div className={`rounded-2xl border p-7 flex flex-col relative transition-all duration-200 ${t.card} ${t.cardHover}`}>
                <div className="absolute -top-[13px] inset-x-0 flex justify-center">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9.5px] font-black uppercase tracking-widest bg-orange-500/15 text-orange-400 border border-orange-500/30 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />7 Day Trial
                  </span>
                </div>
                ...
                <Link href="/signup?plan=trial" className="w-full flex items-center justify-center py-3 rounded-xl text-sm font-bold bg-orange-500/15 text-orange-400 border border-orange-500/30 hover:bg-orange-500/25 hover:border-orange-500/50 transition-all duration-200">Start Trial — $1.99</Link>
              </div>
              <div className={`rounded-2xl border p-7 flex flex-col relative ${d ? "bg-[#140e03] border-orange-500/40" : "bg-white border-orange-400/55"}`} style={{ boxShadow: d ? "0 0 0 1px rgba(249,115,22,0.22), 0 0 60px rgba(249,115,22,0.15)" : "0 0 0 1px rgba(249,115,22,0.18), 0 8px 50px rgba(249,115,22,0.12)" }}>
                <div className="absolute -top-[15px] inset-x-0 flex justify-center">
                  <span className="inline-flex items-center px-4 py-1.5 text-[10px] font-black uppercase tracking-widest bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-full shadow-lg shadow-orange-900/35">Most Popular</span>
                </div>
                <p className="text-[11px] font-black uppercase tracking-[0.14em] mb-4 text-orange-400">Pro</p>
                <div className="mb-6">
                  <div className="flex items-end gap-1.5 mb-1">
                    {annual && <span className={`text-sm font-semibold line-through self-end mb-[5px] mr-0.5 ${t.muted}`}>$12.99</span>}
                    <span className={`text-[2.5rem] font-black tracking-tight leading-none ${d ? "text-white" : "text-gray-900"}`}>${annual ? "10.99" : "12.99"}</span>
                    <span className={`text-sm font-semibold mb-[5px] ${t.muted}`}>/month</span>
                  </div>
                ...
                <Link href="/app" className="w-full flex items-center justify-center py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-orange-500 to-amber-500 text-white shadow-lg shadow-orange-900/30 hover:shadow-orange-900/50 hover:from-orange-400 hover:to-amber-400 transition-all duration-200">Start Pro</Link>
              </div>
              <div className={`rounded-2xl border p-7 flex flex-col transition-all duration-200 ${t.card} ${t.cardHover}`}>
                <p className={`text-[11px] font-black uppercase tracking-[0.14em] mb-4 ${t.muted}`}>Unlimited</p>
                <div className="mb-6">
                  <div className="flex items-end gap-1.5 mb-1">
                    {annual && <span className={`text-sm font-semibold line-through self-end mb-[5px] mr-0.5 ${t.muted}`}>$29.99</span>}
                    <span className={`text-[2.5rem] font-black tracking-tight leading-none ${d ? "text-white" : "text-gray-900"}`}>${annual ? "24.99" : "29.99"}</span>
                    <span className={`text-sm font-semibold mb-[5px] ${t.muted}`}>/month</span>
                  </div>
        ...
        <section className="px-6 pb-28">
          <div className="max-w-[1000px] mx-auto">
            <div className="text-center mb-14">
              ...
              <h2 className={`text-[2.2rem] font-black tracking-tight mb-3 ${d ? "text-white" : "text-gray-900"}`}>Loved by Creators</h2>
              <p className={`text-base ${t.sub}`}>Writers, students, and marketers rely on HumanizeIt every day.</p>
            </div>
            <div className="grid grid-cols-3 gap-4">
              {TESTIMONIALS.map((r) => (
                <div key={r.name} className={`flex flex-col p-7 rounded-2xl border transition-all duration-200 ${t.reviewCard} ${t.reviewHov}`}>
                  <div className="flex gap-0.5 mb-4">{Array.from({ length: 5 }).map((_, i) => <StarIcon key={i} filled={i < r.stars} />)}</div>
                  <p className={`text-[13.5px] leading-[1.85] mb-6 flex-1 ${d ? "text-white/55" : "text-gray-500"}`}>&#8220;{r.quote}&#8221;</p>
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full border flex items-center justify-center text-[12px] font-black ${t.avatar}`}>{r.initials}</div>
                    <div>
        ...
        <section className="px-6 pb-28">
          <div className="max-w-[820px] mx-auto text-center">
            <div className={`rounded-3xl px-10 py-16 border ${t.ctaBanner}`} style={d ? { boxShadow: "0 0 70px rgba(249,115,22,0.09)" } : {}}>
              <h2 className={`text-[2.4rem] font-black tracking-tight mb-4 ${d ? "text-white" : "text-gray-900"}`}>Ready to Sound Human?</h2>
              <p className={`text-lg mb-9 ${t.sub}`}>Start for free. No credit card required.</p>
        ...
        <footer className={`border-t px-6 py-12 ${t.footer}`}>
          <div className="max-w-[1000px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <div className="relative w-7 h-7 rounded-xl bg-gradient-to-br from-orange-400 to-amber-500 flex items-center justify-center">
                  <svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M7 1L8.6 5.6H13.8L9.6 8.5L11.2 13.4L7 10.5L2.8 13.4L4.4 8.5L0.2 5.6H5.4L7 1Z" fill="white"/></svg>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4">
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all duration-200 ${t.socialBtn}`}><InstagramIcon /></a>
              <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" aria-label="TikTok" className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all duration-200 ${t.socialBtn}`}><TikTokIcon /></a>
              <span className={`text-xs ${t.faint}`}>© {new Date().getFullYear()} HumanizeIt. All rights reserved.</span>
            </div>
          </div>
        </footer>
      </div>
